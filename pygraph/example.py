...
def BFS(root, nbrs, offset):
	...
	while(queue):
		...
		#print(currNode)
		start = offset[currNode][0]
		end = offset[currNode+1][0]

		for i in range(start,end):
			if nbrs[i][0] not in visited:
				queue.append(nbrs[i][0])
				visited.append(nbrs[i][0])
				level[nbrs[i][0]] = level[currNode] + 1

				#count = count + 1
		#level.append(count)

	#format is level: # of nodes
	print(collections.Counter(level))
	#print(visited[0:20])
